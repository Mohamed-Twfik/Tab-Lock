...
return (
        <main className="mt-5">
         ...
         <AddUrl />
         <ul className="w-[50%] mx-auto mt-10">
         { 
          data?.urls ? data?.urls.length === 0 ? <h1 className="p-2 font-bold text-center text-red-500 rounded bg-secondary">you have no added domains yet</h1>: data.urls.map((url:string | null, i)=>(
            <URL url={url} key={i} />
            )) 
            : null
            }
